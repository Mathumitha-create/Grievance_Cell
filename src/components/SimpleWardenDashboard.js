...
const SimpleWardenDashboard = ({ user }) => {
  console.log("✅✅✅ SimpleWardenDashboard is rendering for:", user?.email);
  ...
  useEffect(() => {
    console.log("🎯 SimpleWardenDashboard MOUNTED - Fetching hostel complaints");
    ...
    const unsubscribe = onSnapshot(q, 
      (snapshot) => {
        ...
        // Filter for hostel-related complaints
        const hostelComplaints = allComplaints.filter((c) => {
          const category = c.category?.toLowerCase() || '';
          return category.includes('hostel') || 
                 category.includes('mess') || 
                 category.includes('room') ||
                 category.includes('accommodation') ||
                 category.includes('warden');
        });
        
        console.log("✅ Hostel complaints loaded:", hostelComplaints.length);
    ...
    return () => {
      console.log("🧹 Cleaning up Warden dashboard");
      unsubscribe();
    };
  }, []);
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial', minHeight: '100vh', background: '#f3f4f6' }}>
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
          <div>
            <h1 style={{ margin: '0 0 10px 0', fontSize: '2rem' }}>🏠 Warden Dashboard</h1>
            <p style={{ margin: '0', opacity: 0.9, fontSize: '1rem' }}>Email: {user?.email}</p>
            <p style={{ margin: '5px 0 0 0', opacity: 0.8, fontSize: '0.9rem' }}>
              Hostel Department - Manage hostel complaints
            </p>
          </div>
          <button 
      ...
      <div style={{ 
        ...
        <div style={{ background: 'white', padding: '20px', borderRadius: '10px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ color: '#6b7280', marginTop: '5px' }}><TranslatedText text="Total Complaints" /></div>
        </div>
        <div style={{ background: 'white', padding: '20px', borderRadius: '10px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          <div style={{ fontSize: '2rem', fontWeight: 'bold', color: '#f59e0b' }}>{stats.pending}</div>
          <div style={{ color: '#6b7280', marginTop: '5px' }}><TranslatedText text="Pending" /></div>
        </div>
        <div style={{ background: 'white', padding: '20px', borderRadius: '10px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          <div style={{ fontSize: '2rem', fontWeight: 'bold', color: '#8b5cf6' }}>{stats.inProgress}</div>
          <div style={{ color: '#6b7280', marginTop: '5px' }}><TranslatedText text="In Progress" /></div>
        </div>
        <div style={{ background: 'white', padding: '20px', borderRadius: '10px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          <div style={{ fontSize: '2rem', fontWeight: 'bold', color: '#10b981' }}>{stats.resolved}</div>
          <div style={{ color: '#6b7280', marginTop: '5px' }}><TranslatedText text="Resolved" /></div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <h3 style={{ margin: '0 0 15px 0', color: '#1f2937', fontSize: '1.2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '15px' }}>
          <div style={{ textAlign: 'center', padding: '15px', background: '#f9fafb', borderRadius: '8px' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#10b981' }}>{slaCompliance}%</div>
          ...
          <div style={{ textAlign: 'center', padding: '15px', background: '#f9fafb', borderRadius: '8px' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#f59e0b' }}>{slaCounts.critical}</div>
            <div style={{ fontSize: '0.85rem', color: '#6b7280', marginTop: '5px' }}>
              <TranslatedText text="Critical" />
            </div>
          </div>
          <div style={{ textAlign: 'center', padding: '15px', background: '#f9fafb', borderRadius: '8px' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#fbbf24' }}>{slaCounts.warning}</div>
            <div style={{ fontSize: '0.85rem', color: '#6b7280', marginTop: '5px' }}>
              <TranslatedText text="Warning" />
            </div>
          </div>
          <div style={{ textAlign: 'center', padding: '15px', background: '#f9fafb', borderRadius: '8px' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#10b981' }}>{slaCounts.onTrack}</div>
            <div style={{ fontSize: '0.85rem', color: '#6b7280', marginTop: '5px' }}>
      ...
      <div style={{ 
        ...
        {["All", "Pending", "In Progress", "Resolved"].map((status) => (
          <button
            ...
            style={{
              padding: '8px 16px',
              border: filter === status ? '2px solid #f59e0b' : '2px solid #e5e7eb',
              background: filter === status ? '#fef3c7' : 'white',
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280' }}>
            ...
            <p style={{ fontSize: '0.9rem' }}>
              {filter === "All" 
                ? <TranslatedText text="No hostel complaints have been submitted yet" />
                : <TranslatedText text={`No complaints with status "${filter}"`} />}
            </p>
          </div>
        ) : (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '10px', flexWrap: 'wrap', gap: '10px' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '5px', flexWrap: 'wrap' }}>
                      ...
                      <button
                        ...
                        style={{
                          background: speakingId === complaint.id ? '#ef4444' : '#f59e0b',
                          color: 'white',
                          border: 'none',
                          borderRadius: '50%',
                          width: '32px',
                        ...
                        title={speakingId === complaint.id ? t('stop') : t('read_aloud')}
                      >
                        {speakingId === complaint.id ? '⏸️' : '🔊'}
                      </button>
                    </div>
                    <p style={{ margin: '0 0 10px 0', color: '#4b5563', fontSize: '0.95rem' }}>
                  ...
                  >
                    ...
                    <option value="In Progress"><TranslatedText text="In Progress" /></option>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', fontSize: '0.85rem' }}>
                  ...
                  <span style={{
                    background: complaint.status === 'Resolved' ? '#d1fae5' : 
                               complaint.status === 'In Progress' ? '#e0e7ff' : 
                               complaint.status === 'Escalated' ? '#fee2e2' : '#fef3c7',
                    color: complaint.status === 'Resolved' ? '#065f46' : 
                           complaint.status === 'In Progress' ? '#3730a3' : 
                           complaint.status === 'Escalated' ? '#991b1b' : '#92400e',
                    padding: '4px 12px',
                    borderRadius: '4px',
                    fontWeight: '500'
                  }}>
